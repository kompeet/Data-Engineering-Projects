...
stackoverflow as (
    ...
    select
        ...
        timestamp_trunc(min(s.creation_datetime_utc), month) as first_day_of_period,
        extract(month from min(s.creation_datetime_utc)) as month,
...
final as (
    ...
    from stackoverflow s
        right join datespine_join d
            on d.date_quarter = extract(datetime from s.first_day_of_period)
